...
# 从.pth文件中加载预训练参数
densenet.load_state_dict(torch.load(r'model/densenet161.pth',map_location='cpu'))

# 改写网络结构
densenet.fc= nn.Linear(2208,257)
...
#train
log_folder = 'densenet161.txt' # 指定 log 文件夹路径
...
for epoch in range(num_epochs):
    ...
    # 测试模型准确率
    accuracy = test_accuracy(densenet, test_dataloader, device)
    print(f"Epoch {epoch+1} accuracy on train set: {accuracy:.2f}%")

    scheduler.step()

print("Training finished.")
save_path = 'model/densenet161_3.pth'
